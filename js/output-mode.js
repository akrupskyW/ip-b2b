/* =============================================================================
   ...
   docks beside the chat. Charts, graphs and scorecards stay on the reading
   column with that answer. Only a carousel or an image grid cancels the
   column and runs the chat module edge to edge. **As cards** posts an
   output chip there instead and keeps the output itself for the Output pane,
   which opens when the member taps the chip.
...
/* The inline copy borrows the pane's own block styling — it is the same
   ...
   box: no growing, no scroller of its own, and no extra frame. The block sits
   on the reading column with the answer, so a scorecard or a chart is the
   same width as the words above it. Carousels and image grids cancel that
   column themselves (js/transcript-masonry.js, the owl strip, the output
   rail); this block must not do it for them or they would bleed twice. */
