  ...
  /* ══════════════════ WISEcodeAI ══════════════════ */
  {
    ...
    rules: [
      ...
      },
      {
        ...
        how: '<code>chat-activity-strip.js</code> paints a 3px rail on the chat\u2019s <strong>left</strong> edge by default (right is opt-in from the \u22ef menu or Appearance). Every mark on it is an ear-mark, placed at its event\u2019s share of the whole transcript. Gold is an output, green a source, amber a database switch. One output with several versions draws a stacked pair \u2014 two tabs mean \u201cmore than one\u201d, never a count of three or four. Several outputs that share one <strong>chat line</strong> collapse into a single wider ear-mark carrying the number of outputs that combo made, sitting at the mid-point of the block and landing you on its first output; grouping reads the line, not the ask, so nine preview cards on one rail do not ladder into nine tabs, and four outputs that each arrived as their own chat each get an ear-mark. A brand-blue tab with a tiny up-triangle always sits at the bottom of the rail; click it to jump to the top of the last answered prompt. Click a landmark ear-mark to scroll that row into view and flash it. Hover widens the tab and shows the turn ID.',
      },
      {
