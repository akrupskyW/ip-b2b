  ...
  /* ══════════════════ WISEcodeAI ══════════════════ */
  {
    ...
    rules: [
      ...
      },
      {
        ...
        how: '<code>chat-activity-strip.js</code> paints a 3px rail on the chat\u2019s <strong>left</strong> edge by default (right is opt-in from the \u22ef menu or Appearance). Every mark on it is an ear-mark, placed at its event\u2019s share of the whole transcript. Gold is an output, green a source, amber a database switch. One output with several versions draws a stacked pair \u2014 two tabs mean \u201cmore than one\u201d, never a count of three or four. Several outputs from the <strong>same ask</strong> collapse into a single wider ear-mark carrying the number of outputs that ask made, sitting at the mid-point of the block and landing you on its first output; grouping reads the ask stamped on each line, not the per-line turn IDs, so nine preview cards do not ladder into nine tabs. A brand-blue tab with a tiny up-triangle always sits at the bottom of the rail; click it to jump to the top of the last answered prompt. Click a landmark ear-mark to scroll that row into view and flash it. Hover widens the tab and shows the turn ID.',
      },
      {
