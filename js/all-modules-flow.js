...
/* One ear-mark on the rail. `stacked` draws the two-tab multi-version flag,
   `count` draws the collapsed mark that stands for every output one combo made. */
...
const COMPONENTS = [
  ...
  },
  {
    ...
    note: 'An edge-to-edge strip inside the answer: Lottie owls, stills, then silent clips that follow light or dark. Scroll the row or use the arrows — the count at the foot says where you are. Click any owl to open it larger in a panel, then step through the rest the same way. Motion plays once there. No card and no boxed backdrop — the chat surface shows through the gaps. <strong>Carousels and image grids are the only things that run the chat module edge to edge.</strong> Charts, graphs, and scorecards stay on the reading column with the answer.',
