...
export const FEATURE_SIGNALS = [
  ...
  { id: 'ollama', cat: 'features', re: /ollama-chat\.js|web-food-lookup|Clearer reading|data-sc=["']ollama-toggle/,
    on: 'You can ask in your own words. Answers stay on what you asked, then tie in food and nutrition, and name a source when they pull a brand or paper from the web.',
  ...
  { id: 'rowclick', cat: 'ux', re: /product-row-click/,
    on: 'Clicking a product in a table opens that product. Icons, menus, and other controls on the row keep doing their own jobs.',
    off: 'Clicking a product row no longer opens that product.' },
  ...
  { id: 'kebabhover', cat: 'ux', re: /kebab-hover/,
    on: 'Hovering a three-dot menu opens it. You do not have to tap first.',
    off: 'Three-dot menus only open when you tap them.' },
  ...
  { id: 'inlineTbl', cat: 'ui', re: /sc-inline-tbl/,
    on: 'Some answers include a small comparison right in the conversation, separated by thin horizontal lines — not a boxed card and not a separate results board.',
    off: 'Small tables no longer appear in the conversation itself.' },
  { id: 'owlProg', cat: 'components', re: /sc-owl-prog|owl-progression-carousel/,
    on: 'Some answers carry a horizontal owl strip you can scroll — motion, stills, and short clips that follow light or dark, with arrows and a count at the foot. Tap any owl to see it larger, then step through the rest.',
    off: 'The owl progression strip is gone from answers.' },
...
const COMPONENT_MARKERS = [
  ...
  { name: 'Token readout', re: /sc-activity-dots|sc-fb-menu-tokens/ },
...
export const SCRIPT_PURPOSES = {
  ...
  'owl-progression-carousel.js': 'an edge-to-edge owl strip inside an answer — motion, stills, and short clips that follow light or dark, arrows and a count at the foot, and a larger look when you tap one that you can step through',
  'wise-campaign.js': 'a marketing campaign you can ask for — the catalog cast as food characters, the sixteen finished pieces they front, a Food Truth Wins collector set and the two eight-second spots cut for it',
  'think-wise-campaign.js': 'a second campaign you can ask for, built on the owls the app already has now that they have names — Rue doubts the claim, Ollie reads the code, Sage gives the verdict — with the billboards they front, a character bible, the same three owls built as sculpture and installed the length of Golden Gate Park, and the launch film cut from the campaign\u2019s own artwork',
  ...
  'transcript-masonry.js': 'galleries inside an answer that run from edge to edge — a packed grid when the pieces are all different shapes, a scrolling row when they are all the same one, and tapping any of them opens it full size with arrows and a count at the foot to the rest',
  ...
  'trace-stream.js': 'the thinking block a WISEcodeAI turn streams before its answer lands — milestone by milestone, with a running stopwatch',
  ...
  'escape-html.js': 'names and labels are written safely so a product name cannot break the page',
  'toast.js': 'a short confirmation rises from the bottom of the screen when something is saved, sent, or finished',
  'wise-modal.js': 'every dialog uses the same overlay — a fade, a sheet or a centered card, and Escape or the backdrop to close',
  'wise-toolbar.js': 'every list search is the same pill, with the filter funnel inside it when a board has filters',
  'sort-arrow.js': 'every sortable table header uses the same small caret',
  'chat-bridge.js': 'what you do on the board is said back in the chat, as if the assistant did it',
  'owl-mark.js': 'the WISE owl in every chat and welcome circle is the same mark',
...
function labelMap() {
  const map = {
    ...
    '../js/web-food-lookup.js': 'Shared chat',
    ...
    '../js/trace-stream.js': 'Streaming trace',
